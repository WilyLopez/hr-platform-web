              ...
              {data?.results.map((n) => (
                <li
                  ...
                  className={cn(
                    ...
                    !n.leida ? "bg-brand-pale/40 cursor-pointer hover:bg-brand-pale/70" : "bg-white"
                  )}
                >
                  <div className={cn(
                    "w-2 h-2 rounded-full mt-1.5 flex-shrink-0",
                    n.leida ? "bg-neutral-200" : "bg-brand"
                  )} />
                  <div className="flex-1 min-w-0">
                    <p className={cn("text-sm", !n.leida ? "font-semibold text-neutral-900" : "text-neutral-700")}>
                      {n.titulo}
                    </p>
                    <p className="text-xs text-neutral-500 mt-0.5">{n.mensaje}</p>
                    <p className="text-xs text-neutral-400 mt-1">{formatRelative(n.fecha_creacion)}</p>
                  </div>
                </li>
              ))}
            </ul>
